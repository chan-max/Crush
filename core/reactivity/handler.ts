import { isArray, isFunction } from "../common/type"
import { hasOwn, ReactiveFlags, toRaw } from "./common"
...
['includes', 'indexOf', 'lastIndexOf'].forEach((name: string) => {
    arrayHandlers[name] = function (...args: any[]) {
        if (!_isReadonly) { // 非只读才会收集
            console.warn('ARRAY track', name);
        }
        let rawArr = toRaw(this)
        let rawFn = rawArr[name]
        let result = rawFn.apply(rawArr, args)
        return result
    }
});

['push', 'pop', 'shift', 'unshift', 'splice'].forEach((name: string) => {
    arrayHandlers[name] = function (...args: any[]) {
        if (_isReadonly) {
        ...
        let rawFn = rawArr[name]
        let result = rawFn.apply(rawArr, args)
        console.warn('ARRAY trigger', name);
        return result
    }
})
...
export function createSetter(isReadonly: boolean = false, isShallow: boolean = false) {
    return (target: any, key: any, newValue: any, receiver: any) => {
        // 返回 false 时会报错
        if (isReadonly) {
            console.error('READONLY', target, key, newValue);
            return true
        }
